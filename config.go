...
import ("bufio"; "bytes"; "fmt"; "io"; "os"; "path/filepath"; "strings"; "sync"; "time")

// Settings manages server configuration
type Settings struct { mu sync.RWMutex; settings map[string]string }
...
func (s *Settings) Load(filename string) error {
	...
	for scanner.Scan() {
		...
		if len(parts) == 2 { s.settings[strings.TrimSpace(parts[0])] = strings.TrimSpace(parts[1]) }
	}
	return scanner.Err()
}
...
func (s *Settings) GetInt(key string, defaultValue int) int {
	...
	if val, ok := s.settings[key]; ok {
		var result int
		if _, err := fmt.Sscanf(val, "%d", &result); err == nil { return result }
	}
	return defaultValue
}
...
func (s *Settings) GetBool(key string, defaultValue bool) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if val, ok := s.settings[key]; ok { return strings.ToLower(val) == "true" || val == "1" }
	return defaultValue
}

func (s *Settings) Exists(key string) bool { s.mu.RLock(); defer s.mu.RUnlock(); _, ok := s.settings[key]; return ok }
...
type Logger struct { file *os.File; mu sync.Mutex; prefix string; logToFile bool }

func NewLogger(prefix string, logToFile bool) *Logger { return &Logger{prefix: prefix, logToFile: logToFile} }
...
func (l *Logger) Write(format string, args ...interface{}) {
	...
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.file != nil { l.file.WriteString(fullMessage) }
}
...
func NewFileSystem(basePath string) *FileSystem { return &FileSystem{basePath: basePath, cache: make(map[string][]byte)} }

func (fs *FileSystem) SetBasePath(path string) { fs.mu.Lock(); defer fs.mu.Unlock(); fs.basePath = path }
func (fs *FileSystem) GetBasePath() string { fs.mu.RLock(); defer fs.mu.RUnlock(); return fs.basePath }
func (fs *FileSystem) ResolvePath(path string) string { fs.mu.RLock(); defer fs.mu.RUnlock(); return filepath.Join(fs.basePath, path) }

func (fs *FileSystem) FileExists(path string) bool {
	if _, err := os.Stat(fs.ResolvePath(path)); err == nil { return true }
	return false
}

func (fs *FileSystem) LoadFile(path string) ([]byte, error) { return os.ReadFile(fs.ResolvePath(path)) }
...
func (fs *FileSystem) ListDirs(path string) ([]string, error) {
	...
	var dirs []string
	for _, entry := range entries {
		if entry.IsDir() { dirs = append(dirs, entry.Name()) }
	}
	return dirs, nil
}
...
func (fs *FileSystem) FileInfo(path string) (os.FileInfo, error) { return os.Stat(fs.ResolvePath(path)) }
