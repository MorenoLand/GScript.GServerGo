...
func (b *Buffer) WriteGInt4(v uint32) *Buffer {
	...
	val3 := v - (val2 << 7)
	b.data = append(b.data, byte(val0+32), byte(val1+32), byte(val2+32), byte(val3+32))
	b.write = len(b.data)
	return b
}
...
func (b *Buffer) WriteGInt5(v uint64) *Buffer {
	...
	val4 := v - (val3 << 7)
	b.data = append(b.data, byte(val0+32), byte(val1+32), byte(val2+32), byte(val3+32), byte(val4+32))
	b.write = len(b.data)
	return b
}
...
func (b *Buffer) ReadGInt4() uint32 {
	...
	v := uint32(b.data[b.read])<<21 |
		uint32(b.data[b.read+1])<<14 |
		uint32(b.data[b.read+2])<<7 |
		uint32(b.data[b.read+3])
	b.read += 3
	b.read++
	return v - 0x4081020
}
...
func (b *Buffer) ReadGString() string {
	strLen := b.ReadGInt()
	if b.read+int(strLen) > len(b.data) {
		return ""
	}
	start := b.read
	b.read += int(strLen)
...
func (b *Buffer) ReadString() string {
	start := b.read
	for b.read < len(b.data) && b.data[b.read] != 0 {
		b.read++
	}
...
func (e *Encryption) LimitFromType(packetType byte) int {
	...
	for i := 0; i < len(limits); i += 2 {
		if limits[i] == int(packetType) {
			e.limit = int32(limits[i+1])
			return 0
		}
	}
	return 1
...
type SocketManager struct {
	stubs   map[net.Conn]SocketStub
	mu      sync.RWMutex
	running bool
}

func NewSocketManager() *SocketManager {
	return &SocketManager{stubs: make(map[net.Conn]SocketStub), running: false}
}
...
func (sm *SocketManager) Unregister(conn net.Conn) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	if stub, ok := sm.stubs[conn]; ok {
		stub.OnUnregister()
		delete(sm.stubs, conn)
		conn.Close()
	}
}
